...
test('fails from missing preset arg', async (): Promise<void> => {
  const cmd = 'npm run start -- -c busheezy/bushy-packs-test-config -d ./tests/test-output';
  const prom = execAsync(cmd, {
    cwd: path.join(__dirname, '..')
  });
...
test('run', async (): Promise<void> => {
  const cmd = 'npm run start -- -c busheezy/bushy-packs-test-config -d ./tests/test-output -p test';
  const prom = execAsync(cmd, {
    cwd: path.join(__dirname, '..')
  });
...
test('shell & scripting runs', async (): Promise<void> => {
  const cmd = 'npm run start -- -c busheezy/bushy-packs-test-config -d ./tests/test-output -p test -r busheezy/bushy-packs-tests';
  const prom = execAsync(cmd, {
    cwd: path.join(__dirname, '..')
  });
...
test('normal file is copied', async (): Promise<void> => {
  await expect(fileAccess(path.join(testOutputDir, 'file.copy.txt'))).resolves.toBeTruthy();
});

test('fails on file is not there', async (): Promise<void> => {
  await expect(fileAccess(path.join(testOutputDir, 'file.not.here.txt'))).resolves.toBeFalsy();
});

test('ensure file was deleted from delete.txt', async (): Promise<void> => {
  await expect(fileAccess(path.join(testOutputDir, 'file.delete.txt'))).resolves.toBeFalsy();
});

test('folder is copied', async (): Promise<void> => {
  await expect(fileAccess(path.join(testOutputDir, 'folder.copy'))).resolves.toBeTruthy();
  await expect(fileAccess(path.join(testOutputDir, 'folder.copy', 'test1.txt'))).resolves.toBeTruthy();
  await expect(fileAccess(path.join(testOutputDir, 'folder.copy', 'test2.txt'))).resolves.toBeTruthy();
});

test('vdf merged', async (): Promise<void> => {
  const outputBuffer = await fs.readFile(path.join(testOutputDir, 'databases.cfg'));
...
test('handlebars var', async (): Promise<void> => {
  const outputBuffer = await fs.readFile(path.join(testOutputDir, 'file.copy.hb', 'test1.txt'));
  const compareBuffer = await fs.readFile(path.join(compareToDir, 'file.copy.hb', 'test1.txt'));

  expect(outputBuffer).toEqual(compareBuffer);
});

test('handlebars merged var', async (): Promise<void> => {
  const outputBuffer = await fs.readFile(path.join(testOutputDir, 'file.copy.hb', 'test2.txt'));
  const compareBuffer = await fs.readFile(path.join(compareToDir, 'file.copy.hb', 'test2.txt'));

  expect(outputBuffer).toEqual(compareBuffer);
});

test('handlebars incrementer', async (): Promise<void> => {
  const outputBuffer = await fs.readFile(path.join(testOutputDir, 'file.copy.hb', 'test3.txt'));
  const compareBuffer = await fs.readFile(path.join(compareToDir, 'file.copy.hb', 'test3.txt'));

  expect(outputBuffer).toEqual(compareBuffer);
});

test('handlebars condition', async (): Promise<void> => {
  const outputBuffer = await fs.readFile(path.join(testOutputDir, 'file.copy.hb', 'test4.txt'));
  const compareBuffer = await fs.readFile(path.join(compareToDir, 'file.copy.hb', 'test4.txt'));
